...
func TestRefreshTokenSet(t *testing.T) {
	...
	logger := zap.NewNop()
	refreshToken := cache.NewRefreshToken(client, logger)
...
func TestRefreshTokenGetCacheMiss(t *testing.T) {
	...
	assert.Equal(t, cache.ErrCacheMiss, err, "Get should return ErrCacheMiss for non-existent token")
}
...
func TestRefreshTokenDel(t *testing.T) {
	...
	// Verify it's deleted
	_, err = refreshToken.Get(ctx, testToken)
	assert.Equal(t, cache.ErrCacheMiss, err, "Get should return ErrCacheMiss after deletion")
}
